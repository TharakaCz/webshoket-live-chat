...
@else
    ...
    <li class="nav-item">
        @if(Auth::user()->avatar != '' || Auth::user()->avatar != null)
            <a class="nav-link" href="#"><b>Welcome <img src="{{ Auth::user()->avatar_type == "GRAVATAR" ?  Auth::user()->avatar : asset('images/' . Auth::user()->avatar )}}" width="35" class="rounded-circle" />&nbsp; {{ Auth::user()->name }}</b></a>
        @else
