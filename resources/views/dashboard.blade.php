    ...
    <div class="row">
        ...
        <div class="col-sm-4 col-lg-3">
            <div class="card" style="height:255px; overflow-y: scroll;">
                <div class="card-header">
                    <input type="text" class="form-control" placeholder="Search User..." autocomplete="off" id="search_people" onkeyup="search_user('{{ \Illuminate\Support\Facades\Auth::id() }}', this.value);" />
                </div>
                <div class="card-body">
...
<script>
    ...
    var from_user_id = "{{ \Illuminate\Support\Facades\Auth::user()->id }}";
        ...
        if(data.response_load_unconnected_user || data.response_search_user)
        {
            ...
            if(data.data.length > 0)
            {
                ...
                for(var count = 0; count < data.data.length; count++)
                {
                    ...
                    if(data.data[count].avatar != '')
                    {
                        user_image = `<img src="`+data.data[count].avatar+`" width="40" class="rounded-circle" />`;
                    }
                    else
                    {
        ...
        if(data.response_connected_chat_user)
        {
            ...
            if(data.data.length > 0)
            {
				...
				`;
                    ...
                    if(data.data[count].avatar != '' || data.data[count].avatar != null)
                    {
                        user_image = `<img src="`+data.data[count].avatar+`" width="35" class="rounded-circle" />`;
                    }
                    else
                    {
